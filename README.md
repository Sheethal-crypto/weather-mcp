...
Screenshots were captured before the agent was renamed to `weather-agent` and before
`python_exec` was detached, so the header and tool list in the images show the earlier
state. The questions, tool calls, and answers are unchanged.
